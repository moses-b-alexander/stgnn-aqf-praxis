...
table_dir = os.path.join(os.getcwd(), "table/")
...
def make_plots(df, metric, variance, all_plots, agg):
    ...
    if metric == "test_mae":
        ...
        y_tag = "Mean Absolute Error"
    if metric == "number_of_parameters":
        title_tag = "Model Parameter Count"
        y_tag = "Number of Parameters"
    if metric == "training_time_seconds":
        title_tag = "Model Training Time"
        y_tag = "Training Phase Runtime"
    ...
    if all_plots:
        if not variance:
            plot_mae(
                ...
                "model_type", metric, "Model Architecture", y_tag, agg
            )
            plot_mae(
                bottom_5,
        ...
        plot_mae(
            ...
            "index", metric, "Model Class", y_tag, agg
        )
        ...
        if not variance:
            plot_mae(
                ...
                "index", metric, "Model Type", y_tag, agg
            )
            plot_mae(
                mv_posenc_add_type,
...
def full_run(filenames):
    ...
    df = df[["model_type"]+cols]
    df = df.copy()
    df.to_csv(os.path.join(table_dir, f"table_final.csv"))
...
if __name__ == "__main__":
    ...
    # # TODO UNCOMMENT this to produce TABLE_FINAL.CSV and PLOTS.PNGs
